...
func Publish(c *gin.Context, video *models.Video, data *multipart.FileHeader) (err error) {
	...
	//上传视频封面到七牛云
	imgName:= strconv.Itoa(int(video.Id))+".jpeg"
	coverUrl := qiniu.UploadImgToQiNiu(imgName, ImgPath, video.Id)
	video.CoverUrl = coverUrl
	fmt.Println("上传视频封面到七牛云完成")

	// 删除本地视频
	err = os.Remove(VideoPath+`\`+strconv.Itoa(int(video.Id))+".mp4")
	if err!=nil{
		fmt.Println("本地视频删除错误")
	}
	// 删除本地视频封面缩略图
	err = os.Remove(ImgPath+`\`+strconv.Itoa(int(video.Id))+".jpeg")
	if err!=nil{
		fmt.Println(err)
		fmt.Println("本地缩略图删除错误")
	}
